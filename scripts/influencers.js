...
async function inf() {
    for (dem of democrats.list) {
        await db.Tweet.update({ handle: dem },{ '$set': { tag:'democrat', score: 5} }, { upsert: true }).catch(e=>console.error(e))
    }
    for (gop of republicans.list) {
        await db.Tweet.update({ handle: gop },{ '$set': { tag:'republican', score: 5} }, { upsert: true }).catch(e=>console.error(e))
    }
    return;
}
